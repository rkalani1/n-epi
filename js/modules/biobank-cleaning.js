...
(function () {
    ...
    function handleFile(input) {
        ...
        const ok = window.confirm(
            'Use only de-identified or synthetic data in this public demo. Do not upload PHI, patient identifiers, limited datasets, restricted research data, or confidential data. Continue?'
        );
        if (!ok) {
            input.value = '';
            return;
    ...
    function processData(csvText) {
        ...
        data.forEach((row, idx) => {
            ...
            if (idCol && row[idCol]) {
                const id = row[idCol];
                if (idMap[id]) {
                    issues.push(`Line ${lineNum}: [Duplicate] ${idCol} ${redactedValue(idCol)} duplicates line ${idMap[id]}.`);
                } else {
                    idMap[id] = lineNum;
                }
            }
            ...
            const dateCols = Object.keys(row).filter(k => k.includes('date') || k.includes(birthDateToken) || k.includes('time'));
            ...
            const birthDateCol = dateCols.find(c => c.includes(birthDateToken) || c.includes('birth'));
            const visitCol = dateCols.find(c => c.includes('visit') || c.includes('admission') || c.includes('exam'));

            if (birthDateCol && visitCol) {
                const birthDate = parseDate(row[birthDateCol]);
                const visit = parseDate(row[visitCol]);
                if (birthDate && visit && birthDate >= visit) {
                    issues.push(`Line ${lineNum}: [Logic] ${birthDateCol} ${redactedValue(birthDateCol)} is >= ${visitCol} ${redactedValue(visitCol)}.`);
                }
            }
            ...
            if (onsetCol && admitCol) {
                ...
                if (onset && admit && onset > admit) {
                    issues.push(`Line ${lineNum}: [Logic] ${onsetCol} ${redactedValue(onsetCol)} is after ${admitCol} ${redactedValue(admitCol)}.`);
                }
            }
        });
