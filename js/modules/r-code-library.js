            ...
            recipes: [
                {
                    ...
                    code: [
                        ...
                        '             sm = "OR", method.tau = "DL",',
                        '             common = TRUE, random = TRUE)  # meta >= 5.0',
                        '',
                        'summary(m)',
            ...
            recipes: [
                {
                    ...
                    code: [
                        ...
                        'lr_df <- length(lr$n) - 1',
                        'lr_p  <- 1 - pchisq(lr$chisq, df = lr_df)',
                        'cat(sprintf("\\nLog-rank chi-sq = %.3f, df = %d, p = %s\\n",',
                        '            lr$chisq, lr_df, format.pval(lr_p, digits = 4)))',
                        '',
                ...
                },
                {
                    ...
                    desc: 'Subdistribution hazard model when a competing event is present',
                    packages: ['cmprsk', 'survival'],
                    code: [
                        '# Fine-Gray (1999) subdistribution hazard for competing risks.',
                        '# Use when a non-fatal event of interest can be preempted by',
                        '# a competing event (e.g., death from another cause).',
                        'library(cmprsk)',
                        'library(survival)',
                        '',
                        '# Simulated data: time, status (0 = censored, 1 = event of interest,',
                        '# 2 = competing event), and covariates X1/X2',
                        'set.seed(42)',
                        'n <- 400',
                        ...
                        'covars <- cbind(X1, X2)',
                        '',
                        '# Fit Fine-Gray model for the event of interest (failcode = 1).',
                        'fg <- crr(ftime = time, fstatus = status,',
                        '          cov1 = covars, failcode = 1, cencode = 0)',
                        'summary(fg)',
                        '',
                        '# Subdistribution hazard ratios with 95% CI',
                        'sub_hr <- exp(coef(fg))',
                        'ci_lo  <- exp(coef(fg) - 1.96 * sqrt(diag(fg$var)))',
                        'ci_hi  <- exp(coef(fg) + 1.96 * sqrt(diag(fg$var)))',
                        'data.frame(SHR = round(sub_hr, 3),',
                        '           CI_low = round(ci_lo, 3),',
                        '           CI_high = round(ci_hi, 3))',
                        '',
                        '# Cumulative incidence function (CIF) by group',
                        'ci_fit <- cuminc(ftime = time, fstatus = status, group = X2)',
                        'plot(ci_fit, xlab = "Time", ylab = "Cumulative Incidence")',
                        '',
                        '# Note: report BOTH Fine-Gray SHRs and cause-specific HRs',
                        '# (coxph with status==1 alone) when discussing competing risks.',
                        '# See Austin & Fine, Stat Med 2017;36:4391-4400.'
                    ].join('\n')
                }
            ]
        },
            ...
            recipes: [
                {
                    ...
                    code: [
                        ...
                        '',
                        '# Optimal threshold (Youden\'s J). transpose = FALSE works for pROC >= 1.16.',
            ...
            recipes: [
                {
                    ...
                    code: [
                        ...
                        '# Model diagnostics',
                        'cat("\\nAIC:", AIC(fit), "\\n")',
                        '# Discrimination: C-statistic (= AUC) via pROC',
                        'cat("C-statistic (AUC):",',
                        '    round(pROC::auc(outcome, predict(fit, type = "response")), 3), "\\n")',
                        '',
                        '# Calibration: Hosmer-Lemeshow goodness-of-fit (separate from AUC)',
                        '# install.packages("ResourceSelection"); library(ResourceSelection)',
                        '# hl <- hoslem.test(outcome, fitted(fit), g = 10); print(hl)'
                    ].join('\n')
                },
                {
                ...
                {
                    title: 'Modified Poisson for Prevalence Ratios',
                    desc: 'Robust-SE Poisson regression when outcome is common (>10%)',
                    packages: ['sandwich', 'lmtest'],
                    code: [
                        '# Modified Poisson regression with robust (sandwich) variance.',
                        '# Zou (Am J Epidemiol 2004;159:702) showed this gives a',
                        '# consistent estimate of the prevalence/risk ratio even when',
                        '# the outcome is common, without the convergence/over-estimation',
                        '# problems of log-binomial regression.',
                        'library(sandwich)',
                        'library(lmtest)',
                        '',
                        '# Simulated cross-sectional data: y = prevalent disease (binary)',
                        'set.seed(42)',
                        'n <- 800',
                        'x <- rbinom(n, 1, 0.4)         # exposure',
                        'z <- rnorm(n)                  # confounder',
                        'logit_p <- -1 + 0.7 * x + 0.3 * z',
                        'p <- 1 / (1 + exp(-logit_p))   # ~ 30% baseline; >10%',
                        'y <- rbinom(n, 1, p)',
                        'df <- data.frame(y, x, z)',
                        '',
                        '# Fit Poisson with log link; outcome is 0/1',
                        'fit <- glm(y ~ x + z, family = poisson(link = "log"), data = df)',
                        '',
                        '# Robust SEs (sandwich) so the variance is appropriate for binary y',
                        'rob <- coeftest(fit, vcov = sandwich)',
                        'print(rob)',
                        '',
                        '# Prevalence ratios with 95% CI',
                        'b   <- coef(fit)',
                        'se  <- sqrt(diag(sandwich(fit)))',
                        'pr  <- exp(b)',
                        'lo  <- exp(b - 1.96 * se)',
                        'hi  <- exp(b + 1.96 * se)',
                        'data.frame(PR = round(pr, 3),',
                        '           CI_low = round(lo, 3),',
                        '           CI_high = round(hi, 3))'
                    ].join('\n')
                },
                {
                    title: 'E-value (VanderWeele 2017)',
                    desc: 'Sensitivity for unmeasured confounding for OR/RR/HR',
                    packages: ['EValue'],
                    code: [
                        '# VanderWeele & Ding (Ann Intern Med 2017;167:268): the E-value',
                        '# is the minimum strength of association that an unmeasured',
                        '# confounder would need with BOTH the exposure and the outcome,',
                        '# above and beyond measured covariates, to fully explain away the',
                        '# observed effect estimate.',
                        'library(EValue)',
                        '',
                        '# E-value for an OR/RR (rare outcome) or RR-on-the-risk-ratio scale.',
                        'evalues.RR(est = 1.55, lo = 1.20, hi = 2.00)',
                        '',
                        '# E-value for an HR with rare outcome assumption',
                        'evalues.HR(est = 0.72, lo = 0.58, hi = 0.89, rare = TRUE)',
                        '',
                        '# E-value for an OR (rare/common). For common outcomes (>15%),',
                        '# use evalues.OR with rare = FALSE to convert OR to an RR-scale',
                        '# E-value via the square-root rule.',
                        'evalues.OR(est = 2.50, lo = 1.40, hi = 4.50, rare = FALSE)',
                        '',
                        '# Interpretation tip: report the E-value for the point estimate',
                        '# AND for the confidence-interval limit closest to the null.',
                        '# Example: "OR 2.5 (95% CI 1.4-4.5); E-value for the point',
                        '# estimate 4.4 and for the CI limit closest to the null 2.2."'
                    ].join('\n')
                },
                {
                    title: 'MMRM (Mixed-Effects Repeated Measures)',
                    desc: 'Longitudinal continuous outcome with unstructured covariance',
                    packages: ['nlme', 'emmeans'],
                    code: [
                        '# Mixed Model for Repeated Measures (MMRM) is the recommended',
                        '# primary analysis for longitudinal continuous outcomes in many',
                        '# regulatory submissions (e.g., neuroscience trials).',
                        '# Implementation here uses nlme::gls with an unstructured (UN)',
                        '# within-subject covariance. The mmrm package is an alternative.',
                        'library(nlme)',
                        'library(emmeans)',
                        '',
                        '# Long format: id, visit (factor with ordered levels), treatment, y, baseline',
                        '# Sample structure shown; replace with your data.',
                        'set.seed(42)',
                        'id <- factor(rep(1:200, each = 4))',
                        'visit <- factor(rep(c("V1","V2","V3","V4"), 200),',
                        '                levels = c("V1","V2","V3","V4"))',
                        'treatment <- factor(rep(rep(c("Active","Placebo"), each = 4), 100))',
                        'baseline <- rep(rnorm(200, 50, 10), each = 4)',
                        'y <- baseline + as.numeric(visit) * -0.5 +',
                        '     ifelse(treatment == "Active", -2 * as.numeric(visit), 0) +',
                        '     rnorm(800, 0, 5)',
                        'df <- data.frame(id, visit, treatment, baseline, y)',
                        '',
                        '# MMRM via gls with UN covariance; REML by default',
                        'fit <- gls(y ~ baseline + treatment * visit,',
                        '           data = df,',
                        ...
                        'summary(fit)',
                        '',
                        '# Adjusted mean differences at each visit (LSMeans contrast)',
                        'emm <- emmeans(fit, ~ treatment | visit)',
                        'pairs(emm, reverse = TRUE)',
                        '',
                        '# Note: MMRM is restricted-likelihood-based and handles MAR',
                        '# missingness without explicit imputation, unlike LOCF.'
                    ].join('\n')
                }
            ]
        }
