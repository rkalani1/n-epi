...
(function() {
    ...
    var formulasDB = [
        ...
        { id: 'ss-two-prop', category: 'Sample Size', name: 'Two Proportions', formula: 'n = (z_alpha + z_beta)^2 * [p1(1-p1) + p2(1-p2)] / (p1 - p2)^2', definition: 'Sample size per group for comparing two independent proportions.', interpretation: 'Increase n for: smaller difference, smaller alpha, higher power, proportions near 0.5.', example: 'p1=0.30, p2=0.20, alpha=0.05 (z=1.96), power=0.80 (z=0.84): n = 7.84*(0.30*0.70+0.20*0.80)/0.01 = 7.84*0.37/0.01 = 290.08 -> 291 per group' },
        ...
        { id: 'ci-or', category: 'Confidence Interval', name: 'CI for Odds Ratio', formula: 'exp[ln(OR) +/- z * sqrt(1/a + 1/b + 1/c + 1/d)]', definition: 'Confidence interval for an odds ratio using the log transformation.', interpretation: 'If CI includes 1, OR is not statistically significant. Asymmetric on natural scale.', example: 'a=30, b=20, c=15, d=35: OR = (30*35)/(20*15) = 3.50; SE(lnOR) = sqrt(1/30+1/20+1/15+1/35) = 0.423; 95% CI = exp(ln(3.5) +/- 1.96*0.423) = (1.53, 8.02)' },
        { id: 'bayes-theorem', category: 'Diagnostic', name: "Bayes' Theorem (diagnostic)", formula: 'Post-test odds = Pre-test odds * LR', definition: 'Update probability of disease after a test result using likelihood ratios.', interpretation: 'Convert prevalence to pre-test odds, multiply by LR+ (if positive) or LR- (if negative), convert back to probability.', example: 'Prevalence=10% -> pre-test odds = 0.10/0.90 = 0.111. LR+=6.0 -> post-test odds = 0.667 -> post-test probability = 0.667/1.667 = 40%' },
