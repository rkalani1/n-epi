...
(function() {
    ...
    function renderDataTable() {
        const el = document.getElementById('ma-data-table');
        if (!el) return;
        let html = '';
    ...
    function loadExample() {
        selectedMeasure = 'OR';
        const measureSel = document.getElementById('ma-measure');
    ...
    function prepareEffects() {
        ...
        const seArr = [];
        const subgroups = [];
        const isLogScale = (selectedMeasure === 'OR' || selectedMeasure === 'RR' || selectedMeasure === 'HR');

        if (inputMode === 'effect') {
            for (let i = 0; i < studyData.length; i++) {
                const s = studyData[i];
                const eff = parseFloat(s.logEffect);
                if (isNaN(eff)) continue;

                let se = parseFloat(s.se);
                if (isNaN(se) || se <= 0) {
                    const lo = parseFloat(s.ciLower);
                    const hi = parseFloat(s.ciUpper);
            ...
            for (let j = 0; j < studyData.length; j++) {
                const d = studyData[j];
                const e1 = parseInt(d.e1);
                const n1 = parseInt(d.n1);
                const e2 = parseInt(d.e2);
                const n2 = parseInt(d.n2);
                if (isNaN(e1) || isNaN(n1) || isNaN(e2) || isNaN(n2)) continue;
                if (n1 <= 0 || n2 <= 0) continue;

                const a = e1, b = n1 - e1, c = e2, dv = n2 - e2;
                let cc = 0;
                if (a === 0 || b === 0 || c === 0 || dv === 0) cc = 0.5;

                let eff2, se2;
                if (selectedMeasure === 'OR') {
                    const orVal = ((a + cc) * (dv + cc)) / ((b + cc) * (c + cc));
                    eff2 = Math.log(orVal);
                    se2 = 1 / (a + cc) + 1 / (b + cc) + 1 / (c + cc) + 1 / (dv + cc);
                } else if (selectedMeasure === 'RR') {
                    const rrVal = ((a + cc) / (n1 + 2 * cc)) / ((c + cc) / (n2 + 2 * cc));
                    eff2 = Math.log(rrVal);
                    ...
                    const orV2 = ((a + cc) * (dv + cc)) / ((b + cc) * (c + cc));
                    eff2 = Math.log(orV2);
                    se2 = 1 / (a + cc) + 1 / (b + cc) + 1 / (c + cc) + 1 / (dv + cc);
                }
    ...
    function runAnalysis() {
        const prep = prepareEffects();
        if (prep.effects.length < 2) {
            Export.showToast('Need at least 2 studies with valid data', 'error');
            return;
        }

        const k = prep.effects.length;
        const isLog = prep.isLogScale;

        const fe = Statistics.metaAnalysisFixedEffect(prep.effects, prep.variances);
        const re = Statistics.metaAnalysisRandomEffects(prep.effects, prep.variances, { hksj: useHKSJ });
        const egger = k >= 3 ? Statistics.eggerTest(prep.effects, prep.se) : null;
        const loo = Statistics.leaveOneOut(prep.effects, prep.variances);
        const cum = Statistics.cumulativeMA(prep.effects, prep.variances, prep.names);

        const reWeights = re.weights;

        const forestStudies = prep.names.map(function(name, i) {
            return {
                name: name,
        ...
        const i2val = re.I2 * 100;
        const i2interp = i2val < 25 ? 'low' : (i2val < 50 ? 'moderate' : (i2val < 75 ? 'substantial' : 'considerable'));
        const hetSig = re.pHet < 0.10 ? 'significant' : 'not significant';
        ...
        const resultEl = document.getElementById('ma-results');
        ...
        const reEst = isLog ? Math.exp(re.pooled).toFixed(3) : re.pooled.toFixed(3);
        const reLo = isLog ? Math.exp(re.ci.lower).toFixed(3) : re.ci.lower.toFixed(3);
        const reHi = isLog ? Math.exp(re.ci.upper).toFixed(3) : re.ci.upper.toFixed(3);
        const feEst = isLog ? Math.exp(fe.pooled).toFixed(3) : fe.pooled.toFixed(3);
        const feLo = isLog ? Math.exp(fe.ci.lower).toFixed(3) : fe.ci.lower.toFixed(3);
        const feHi = isLog ? Math.exp(fe.ci.upper).toFixed(3) : fe.ci.upper.toFixed(3);
        ...
        if (re.predInterval) {
            const piLo = isLog ? Math.exp(re.predInterval.lower).toFixed(3) : re.predInterval.lower.toFixed(3);
            const piHi = isLog ? Math.exp(re.predInterval.upper).toFixed(3) : re.predInterval.upper.toFixed(3);
        ...
        loo.forEach(function(r) {
            const est = isLog ? Math.exp(r.pooled).toFixed(3) : r.pooled.toFixed(3);
            const lo = isLog ? Math.exp(r.ci.lower).toFixed(3) : r.ci.lower.toFixed(3);
            const hi = isLog ? Math.exp(r.ci.upper).toFixed(3) : r.ci.upper.toFixed(3);
        ...
        // Results text
        const resultsText = generateResultsText(prep, fe, re, egger, isLog);
    ...
    function runSubgroupAnalysis() {
        ...
        const varEl = document.getElementById('ma-subgroup-var');
        subgroupVariable = varEl ? varEl.value : 'Subgroup';

        // Check that we have subgroup data
        const hasSubgroups = prep.subgroups.some(function(s) { return s && s.trim() !== ''; });
        if (!hasSubgroups) {
            Export.showToast('No subgroup labels found. Please fill in the Subgroup column in the data table.', 'error');
            return;
        }

        const groups = prep.subgroups.map(function(s) { return s && s.trim() !== '' ? s.trim() : 'Unspecified'; });
        const subResult = Statistics.subgroupAnalysis(prep.effects, prep.variances, groups);
        const isLog = prep.isLogScale;

        const resultEl = document.getElementById('ma-subgroup-results');
        let html = '<div class="result-panel animate-in mt-3">';
        ...
        const subgroupKeys = Object.keys(subResult.subgroups);
        subgroupKeys.forEach(function(key) {
            const sg = subResult.subgroups[key];
            const nStudies = prep.effects.filter(function(_, i) { return groups[i] === key; }).length;
            const est = isLog ? Math.exp(sg.pooled).toFixed(3) : sg.pooled.toFixed(3);
            const lo = isLog ? Math.exp(sg.ci.lower).toFixed(3) : sg.ci.lower.toFixed(3);
            const hi = isLog ? Math.exp(sg.ci.upper).toFixed(3) : sg.ci.upper.toFixed(3);
        ...
        // Overall
        const overEst = isLog ? Math.exp(subResult.overall.pooled).toFixed(3) : subResult.overall.pooled.toFixed(3);
        const overLo = isLog ? Math.exp(subResult.overall.ci.lower).toFixed(3) : subResult.overall.ci.lower.toFixed(3);
        const overHi = isLog ? Math.exp(subResult.overall.ci.upper).toFixed(3) : subResult.overall.ci.upper.toFixed(3);
        ...
        setTimeout(function() {
            subgroupKeys.forEach(function(key, ki) {
                const canvas = document.getElementById('ma-subforest-' + ki);
                if (!canvas) return;
                const idx = [];
                groups.forEach(function(g, i) { if (g === key) idx.push(i); });

                const studies = idx.map(function(i) {
                    return {
                        name: prep.names[i],
                        estimate: prep.effects[i],
                        ci: {
    ...
    function copySubgroupResults() {
        const el = document.getElementById('ma-subgroup-results');
    ...
    function runPublicationBias() {
        ...
        const isLog = prep.isLogScale;
        const re = Statistics.metaAnalysisRandomEffects(prep.effects, prep.variances, { hksj: useHKSJ });
        const egger = Statistics.eggerTest(prep.effects, prep.se);

        // Begg's rank correlation (Kendall's tau)
        const begg = computeBeggTest(prep.effects, prep.se);

        // Trim and fill
        const tf = Statistics.trimAndFill(prep.effects, prep.variances);

        const resultEl = document.getElementById('ma-pubbias-results');
        let html = '<div class="result-panel animate-in mt-3">';
        ...
        const eggerInterp = egger.pValue < 0.10
        ...
        const beggInterp = begg.pValue < 0.10
        ...
        const origEst = isLog ? Math.exp(tf.original.pooled).toFixed(3) : tf.original.pooled.toFixed(3);
        const adjEst = isLog ? Math.exp(tf.adjusted.pooled).toFixed(3) : tf.adjusted.pooled.toFixed(3);
        const adjLo = isLog ? Math.exp(tf.adjusted.ci.lower).toFixed(3) : tf.adjusted.ci.lower.toFixed(3);
        const adjHi = isLog ? Math.exp(tf.adjusted.ci.upper).toFixed(3) : tf.adjusted.ci.upper.toFixed(3);
        ...
        setTimeout(function() {
            const canvas = document.getElementById('ma-tf-funnel-canvas');
            if (!canvas) return;

            const imputedEffects = tf.imputedEffects;
            const imputedSE = tf.imputedVariances.map(function(v) { return Math.sqrt(v); });
    ...
    function computeBeggTest(effects, se) {
        ...
        for (let i = 0; i < k; i++) {
            for (let j = i + 1; j < k; j++) {
                const diffEffect = effects[i] - effects[j];
                const diffSE = se[i] - se[j];
    ...
    function copyPubBiasResults() {
        const el = document.getElementById('ma-pubbias-results');
        if (el) Export.copyText(el.textContent);
    }
    ...
    function copyMetaRegCode() {
        const code = '# Meta-regression with metafor\nlibrary(metafor)\n\n'
    ...
    function computeGRADE() {
        const base = parseInt(document.getElementById('ma-grade-base').value);
        let level = base;

        const downDomains = ['rob', 'inconsistency', 'indirectness', 'imprecision', 'pubbias'];
        const downRatings = {};
        downDomains.forEach(function(d) {
            const sel = document.querySelector('input[name="ma-grade-' + d + '"]:checked');
            if (sel) {
        ...
        const upDomains = ['large', 'dose', 'confounding'];
        const upRatings = {};
        upDomains.forEach(function(d) {
            const sel = document.querySelector('input[name="ma-grade-up-' + d + '"]:checked');
            if (sel) {
        ...
        const resultEl = document.getElementById('ma-grade-result');
    ...
    function addGRADEOutcome() {
        const outcome = document.getElementById('ma-grade-outcome').value || 'Unnamed outcome';
        const nStudies = document.getElementById('ma-grade-nstudies').value || '?';
        const notes = document.getElementById('ma-grade-notes').value || '';
    ...
    function renderGRADETable() {
        const el = document.getElementById('ma-grade-table');
    ...
    function generatePRISMA() {
        const fields = ['identified', 'otherSources', 'duplicates', 'screened', 'excludedScreen', 'fullText', 'excludedFT', 'excludedReasons', 'includedQual', 'includedQuant'];
        const vals = {};
        fields.forEach(function(f) {
            const el = document.getElementById('ma-prisma-' + f);
            vals[f] = el ? el.value.trim() : '';
            prismaNumbers[f] = vals[f];
        });

        const n = function(v) { return v || 'n'; };
        ...
        if (vals.excludedReasons) {
            const reasons = vals.excludedReasons.split(',');
        ...
        const outputEl = document.getElementById('ma-prisma-output');
    ...
    function copyPRISMA() {
        const el = document.getElementById('ma-prisma-output');
        if (el) {
            const pre = el.querySelector('.text-output');
    ...
    function drawForestPlot(studies, re, prep, isLog) {
        const canvas = document.getElementById('ma-forest-canvas');
        if (!canvas) return;
    ...
    function drawFunnelPlot(prep, re, egger) {
        const canvas = document.getElementById('ma-funnel-canvas');
        if (!canvas) return;
    ...
    function generateResultsText(prep, fe, re, egger, isLog) {
        const k = prep.effects.length;
        const reEst = isLog ? Math.exp(re.pooled).toFixed(2) : re.pooled.toFixed(2);
        const reLo = isLog ? Math.exp(re.ci.lower).toFixed(2) : re.ci.lower.toFixed(2);
        const reHi = isLog ? Math.exp(re.ci.upper).toFixed(2) : re.ci.upper.toFixed(2);
        const feEst = isLog ? Math.exp(fe.pooled).toFixed(2) : fe.pooled.toFixed(2);
        const i2val = (re.I2 * 100).toFixed(1);
        const i2interp = re.I2 * 100 < 25 ? 'low' : (re.I2 * 100 < 50 ? 'moderate' : (re.I2 * 100 < 75 ? 'substantial' : 'considerable'));
        ...
        if (re.predInterval) {
            const piLo = isLog ? Math.exp(re.predInterval.lower).toFixed(2) : re.predInterval.lower.toFixed(2);
            const piHi = isLog ? Math.exp(re.predInterval.upper).toFixed(2) : re.predInterval.upper.toFixed(2);
    ...
    function copyAllResults() {
        const textEl = document.getElementById('ma-results-text');
        if (textEl) {
