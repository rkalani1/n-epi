...
var RGenerator = (function() {
    ...
    var sampleSize = {
        ...
        twoProportions: function(params) {
            ...
            script += tryCatchWrap([
                ...
                '',
                '  # Adjust for unequal allocation (ratio = n_treatment / n_control):',
                '  # total N inflates by (1 + ratio)^2 / (4 * ratio) relative to 1:1',
                '  if (ratio != 1) {',
                '    n_control <- ceiling(n_per_group * (1 + 1 / ratio) / 2)',
                '    n_treatment <- ceiling(n_per_group * (1 + ratio) / 2)',
        ...
        survival: function(params) {
            ...
            script += tryCatchWrap([
                ...
                '  # (general allocation: D = (za+zb)^2 / (p1*p2*(log hr)^2), p1*p2 = ratio/(1+ratio)^2)',
                '  events_needed <- ceiling((z_alpha + z_beta)^2 * (1 + ratio)^2 / (ratio * (log(hr))^2))',
                '',
                '  # Freedman formula (alternative): D = (za+zb)^2 * (1 + ratio*hr)^2 / (ratio * (1-hr)^2)',
        ...
        nonInferiority: function(params) {
            ...
            script += tryCatchWrap([
                ...
                '',
                '  if (design == "equiv") {',
                '    # Two one-sided tests (TOST): power is split across both bounds,',
                '    # so use z at 1 - (1 - power)/2 instead of z at power',
                '    z_beta_equiv <- qnorm(1 - (1 - power) / 2)',
                '    n_per_group <- ceiling(((z_alpha + z_beta_equiv)^2 * 2 * p * (1 - p)) / margin^2)',
                '  } else {',
                '    # Non-inferiority',
    ...
    function nntCalculator(params) {
        ...
        script += tryCatchWrap([
            ...
            '  fisher <- fisher.test(tab)',
            '',
            '  # Confidence interval for ARR (Wald interval from prop.test;',
            '  # for the Newcombe hybrid-score interval use DescTools::BinomDiffCI(method = "score"))',
            '  prop_test <- prop.test(c(c, a), c(nc, ne), correct = FALSE)',
            '  cat("ARR 95% CI:", round(prop_test$conf.int[1], 4), "to", round(prop_test$conf.int[2], 4), "\\n")',
            '',
            '  # --- Results ---',
            ...
            '',
            '  # Fragility Index (Walsh et al. 2014): convert non-events to events',
            '  # in the group with FEWER events until significance is lost',
            '  frag_idx <- 0',
            ...
            '      }',
            '      frag_idx <- frag_idx + 1',
            '      if (frag_idx > 1000 || bb < 0 || dd < 0) break',
            '    }',
            '  } else {',
            '    cat("Result is non-significant; the fragility index is defined for significant results.\\n")',
            '  }',
            ...
            '    if (eer_count <= cer_count) {',
            '      # Treatment reduces events (benefit)',
            '      categories <- c(rep("Event despite treatment", eer_count),',
            '                      rep("Benefit from treatment", cer_count - eer_count),',
            '                      rep("No event regardless", n_icons - cer_count))',
            '    } else {',
            '      # Treatment increases events (harm)',
            '      categories <- c(rep("Event regardless", cer_count),',
            '                      rep("Harm from treatment", eer_count - cer_count),',
            '                      rep("No event regardless", n_icons - eer_count))',
            '    }',
            '    icon_df <- data.frame(',
            '      x = rep(1:10, each = 10),',
            '      y = rep(10:1, times = 10),',
            '      category = factor(categories, levels = unique(categories))',
            '    )',
            '',
